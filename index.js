...
const contributions = () => {
	return GAMES_JSON.reduce((a, c) => {
		return a + c.backers
	}, 0)
}

contributionsCard.innerHTML = `
    <h3>${contributions().toLocaleString()}</h3>`
...
const unfundedDescription = () => {
    ...
    const unfundedGamesString = unfundedGamesCount === 1 ? "game" : "games"
    const unfundedGamesDescription = document.createElement("p")
    unfundedGamesDescription.innerHTML = `
        <p>There are currently ${unfundedGamesCount} unfunded ${unfundedGamesString} on the site.</p>
    `
